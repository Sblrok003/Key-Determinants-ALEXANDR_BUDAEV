"""
ETH Stage 3.0 — Final Walk-Forward Stability Check
==================================================

Что делает:
1. Берёт финальные режимы из Stage 2.1:
   - horizon
...
Запуск:
    python eth_3_0_walk_forward_final_check.py
"""
...
# Основная папка второй стадии ETH.
STAGE2_DIR = BASE_DIR / "eth_2_1_joint_model_outputs_fast_selection_final_heavy"
...
OUTPUT_DIR = BASE_DIR / "eth_3_0_walk_forward_final_check_outputs"
...
def resolve_stage2_paths():
    """
    Проверяет, что папка второй стадии ETH найдена.
    ...
    print("WARNING: стандартная папка Stage 2 не найдена:")
    print(STAGE2_DIR)
    ...
    if not candidates:
        raise FileNotFoundError(
            ...
            "Проверь, что Stage 2.1 для ETH уже был запущен."
        )

    # Предпочитаем путь, где есть eth в названии папки.
    eth_candidates = [
        p for p in candidates
        if "eth" in str(p.parent).lower()
    ]

    chosen = eth_candidates[0] if eth_candidates else candidates[0]
...
def make_plots(wf_summary, conf_summary, comparison):
    ...
    if wf_summary is not None and not wf_summary.empty:
        ...
        save_bar_top(
            ...
            "mean_roc_auc",
            "regime_model",
            "ETH Stage 3.0 walk-forward: top regimes by mean ROC-AUC",
        ...
        save_bar_top(
            ...
            "regime_model",
            "ETH Stage 3.0 walk-forward: top regimes by mean balanced accuracy",
        ...
        if not heat_data.empty:
            ...
            plt.title("ETH Stage 3.0 WF heatmap: horizon x threshold")
        ...
        plt.title("ETH Stage 3.0 stability flags")
    ...
    if conf_summary is not None and not conf_summary.empty:
        ...
        save_bar_top(
            ...
            "regime_model",
            "ETH Stage 3.0 confidence: top regimes by signal accuracy",
        ...
        plt.title("ETH Stage 3.0 confidence: mean signal accuracy vs coverage")
    ...
    if comparison is not None and not comparison.empty:
        ...
        if not comp.empty:
            ...
            plt.title("ETH Stage 2 validation ROC-AUC vs Stage 3 walk-forward ROC-AUC")
...
def save_excel(
    ...
    xlsx_path = OUTPUT_DIR / "ETH_STAGE_3_0_WALK_FORWARD_FINAL_CHECK.xlsx"
    ...
    readme = pd.DataFrame({
        ...
        "text": [
            "Stage 3.0 — заключительная проверка устойчивости финальных режимов Stage 2.1 для ETH.",
            ...
            "Это не доказывает торговую прибыльность. Это проверка качества ML-сигнала до торгового backtest.",
        ]
    })
...
def write_summary_txt(wf_summary, conf_summary, recommended, warnings_list, plot_files, excel_path):
    path = OUTPUT_DIR / "ETH_STAGE_3_0_SUMMARY.txt"

    with open(path, "w", encoding="utf-8") as f:
        f.write("ETH STAGE 3.0 — WALK-FORWARD FINAL CHECK\n")
        f.write("=" * 80 + "\n\n")

        f.write("Главное:\n")
        f.write("- Признаки НЕ подбирались заново.\n")
        f.write("- Использованы final_features из Stage 2.1.\n")
...
def main():
    print("=" * 80)
    print("ETH Stage 3.0 — Walk-Forward Final Check")
    print("=" * 80)
    ...
    print("- ETH_STAGE_3_0_WALK_FORWARD_FINAL_CHECK.xlsx")
    print("- ETH_STAGE_3_0_SUMMARY.txt")
