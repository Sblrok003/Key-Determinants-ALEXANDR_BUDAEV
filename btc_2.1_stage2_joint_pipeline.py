...
OUTPUT_DIR = BASE_DIR / "btc_2_1_joint_model_outputs_fast_selection_final_heavy"
PLOTS_DIR = OUTPUT_DIR / "plots"

# Файл-закваска для второго этапа
CANDIDATE_POOL_PATH = (
    BASE_DIR
    / "btc_1_1_baseline_feature_test_outputs"
    / "candidate_pool_for_joint_stage.xlsx"
)

# Главный лист с кандидатами по horizon + threshold
CANDIDATE_POOL_SHEET = "final_nested_h_thr"
...
# ВАЖНО: пороги не смешиваем
DO_NOT_MIX_THRESHOLDS = True

# CPI не берём автоматически в следующий этап
EXCLUDED_FEATURES = {
    "us_cpi_inflation",
}
...
# SPEED LOGIC
# ============================================================
# Во время отбора признаков считаются только быстрые модели:
# LogisticRegression, RandomForest, ExtraTrees.
#
# Тяжёлые модели считаются ТОЛЬКО в конце, когда финальный набор признаков
# уже выбран для конкретного horizon + threshold.
# ============================================================
...
# LSTM не участвует в отборе признаков. В этой версии она считается только в финале.
...
# Модели, которые управляют отбором признаков.
# XGBoost / CatBoost / MLP / LSTM здесь НЕ используются.
SELECTION_MODELS = [
    "LogisticRegression",
    "RandomForest",
    "ExtraTrees",
...
# Модели, которые считаются только на финальном наборе признаков.
FINAL_MODEL_NAMES = [
    "LogisticRegression",
    "RandomForest",
    "ExtraTrees",
...
# Confidence-selected режимы в финале выбираются по всем финальным моделям,
# которые реально посчитались. Если хочешь отбирать confidence только по быстрым,
# поменяй на SELECTION_MODELS.copy().
...
RENAME_MAP = {
    ...
    "btc_mcap_log_return": "btc_mcap_log_return",
    "BTC_mcap_log_return": "btc_mcap_log_return",
...
GROUP_RU = {
    "SET0": "базовые признаки цены и волатильности",
    "market": "рыночные признаки",
    "behavior": "поведенческие признаки",
    "onchain": "ончейн-признаки",
...
class TorchLSTMClassifier:
    """
    sklearn-like wrapper для LSTM.

    X — обычная табличная матрица по датам.
    Внутри она превращается в последовательности:
        [t-seq_len+1, ..., t] -> y_t

    Поэтому эффективное число наблюдений меньше:
        n_effective = n_rows - seq_len + 1

    Первые seq_len-1 прогнозов возвращаются как NaN и не участвуют в метриках.
    """
...
def load_candidate_pool():
    ...
    if not path.exists():
        found = find_file(BASE_DIR, "candidate_pool_for_joint_stage.xlsx")
        if found is not None:
            path = found

    if not path.exists():
        raise FileNotFoundError(
            "Не найден candidate_pool_for_joint_stage.xlsx. "
            "Положи его в папку btc_1_1_baseline_feature_test_outputs."
        )
    ...
    cand["target_threshold_pct"] = pd.to_numeric(
        cand["target_threshold_pct"],
        errors="coerce"
    ).astype("Int64")
    ...
    # Пересобираем группу по словарю кода
...
def get_models(y_train, model_scope="selection"):
    """
    model_scope:
        "selection" -> быстрые модели для отбора признаков
        "final"     -> все модели только на финальном наборе признаков

    ВАЖНО:
    На этапах отбора признаков тяжелые модели НЕ считаются.
    XGBoost / CatBoost / MLP / LSTM включаются только в финале.
    """
    ...
    # Быстрые модели: используются и в selection, и в final
    ...
    # На отборах признаков возвращаем только быстрые модели.
    if model_scope == "selection":
        return models

    # Ниже — тяжелые модели, только для model_scope="final".
    if USE_MLP:
...
def aggregate_validation(rows, model_names=None):
    """
    Эта функция агрегирует validation-метрики.
    По умолчанию используется для отбора признаков и берёт только SELECTION_MODELS.
    """
...
def run_final_all_models_stage(df, final_features, target, horizon, threshold_pct):
    """
    Финальный прогон всех моделей.

    В этот этап попадают:
    - LogisticRegression
    - RandomForest
    - ExtraTrees
    - XGBoost
    ...
    Этот этап НЕ выбирает признаки.
    Он только проверяет уже выбранный финальный набор.
    """
...
def run_confidence_stage(df, final_features, target, horizon, threshold_pct):
    """
    Confidence считается только в конце и только на финальном наборе признаков.
    Здесь тяжелые модели включены.
    """
...
def run_stage2_pipeline(df, candidate_pool):
    ...
    for h in HORIZONS:
        for thr in TARGET_THRESHOLDS:
            ...
            print("\n" + "=" * 80)
            print(f"Running BTC stage 2.1 | horizon={h} | threshold={pct}%")
            print("=" * 80)
            ...
            # 1. SET0 + groups. Только быстрые модели.
            ...
            # 2. Forward selection внутри выбранных групп. Только быстрые модели.
            if selected_groups:
            ...
            # 4. Regime features. Только быстрые модели.
            ...
            # 5. Interaction features. Только быстрые модели.
            ...
            # 6. Combined all selected blocks. Только быстрые модели.
            ...
            # 7. Финальный прогон всех моделей. Здесь heavy models ON.
            ...
            # 8. Confidence / assurance. Здесь heavy models ON.
...
def write_excel_report(outputs):
    xlsx_path = OUTPUT_DIR / "2_1_BTC_joint_model_report_fast_selection_final_heavy.xlsx"
    ...
    with pd.ExcelWriter(xlsx_path, engine="openpyxl") as writer:
        readme = pd.DataFrame({
            ...
            "text": [
                "Это второй этап: joint-модели по кандидатам из закваски.",
...
def write_text_summary(outputs):
    ...
    with open(path, "w", encoding="utf-8") as f:
        f.write("BTC — STAGE 2.1 JOINT MODEL PIPELINE\n")
        ...
        if eval_df is not None and not eval_df.empty:
            ...
            if not lstm_rows.empty:
                f.write("LSTM diagnostics:\n")

                errors = lstm_rows[lstm_rows["eval_split"].astype(str).str.contains("error", na=False)]

                f.write(f"- LSTM total rows: {len(lstm_rows)}\n")
                f.write(f"- LSTM error rows: {len(errors)}\n")

                ok = lstm_rows[lstm_rows["eval_split"].isin(["validation", "test"])].copy()

                if not ok.empty:
                    f.write(f"- LSTM successful eval rows: {len(ok)}\n")
                    f.write(f"- Avg effective train sequences: {ok['lstm_effective_train_sequences'].mean()}\n")
                    f.write(f"- Avg effective validation sequences: {ok['lstm_effective_val_sequences'].mean()}\n")
                    f.write(f"- Avg effective test sequences: {ok['lstm_effective_test_sequences'].mean()}\n")

                f.write(
                    "Interpretation: LSTM reduces effective sample size because each observation requires "
                    "a rolling sequence. This is one reason deep sequence models may be unstable on this dataset.\n\n"
                )
        ...
        f.write("- 2_1_BTC_joint_model_report_fast_selection_final_heavy.xlsx\n")
...
def make_plots(outputs):
    ...
    if final_plan is not None and not final_plan.empty:
        ...
        plt.title("BTC stage 2.1: top regimes by validation score after combined stage")
    ...
    if confidence_selected is not None and not confidence_selected.empty:
        ...
        plt.title("BTC confidence regimes: validation accuracy vs coverage")
        plt.xlabel("Coverage")
...
def main():
    ...
    print("BTC Stage 2.1 joint pipeline")
    ...
    print("\nMain files:")
    print("- 2_1_BTC_joint_model_report_fast_selection_final_heavy.xlsx")
