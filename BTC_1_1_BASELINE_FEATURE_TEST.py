...
OUTPUT_DIR = Path(FILE_PATH).parent / "btc_1_1_baseline_feature_test_outputs"
...
# Пороги значимого движения
# 0.00 = обычный рост/падение
# 0.01 = рост больше +1% / падение ниже -1%
# 0.03 = рост больше +3% / падение ниже -3%
TARGET_THRESHOLDS = [0.00, 0.01, 0.02, 0.03, 0.05]

# Минимум строк, чтобы эксперимент считался нормальным
MIN_TRAIN_ROWS = 100
MIN_VAL_ROWS = 50
MIN_TEST_ROWS = 50

# По каким моделям делать отбор признаков
# Baseline отдельно, для отбора лучше не использовать
SELECTION_MODELS = [
    "LogisticRegression",
    "RandomForest",
    "ExtraTrees",
...
# Можно включить, если установлены xgboost/catboost
USE_XGBOOST = True
USE_CATBOOST = True
...
def fit_eval_models(data, features, target, experiment_type, feature_set_name, extra=None):
    ...
    for model_name, model in models.items():
        # Baseline можно обучать на dummy X.
        # Остальные модели требуют признаки.
...
def run_baseline_and_base(df):
    ...
    for h in HORIZONS:
        for thr in TARGET_THRESHOLDS:
            ...
            # Для baseline и SET0 берём данные только по SET0.
            data = make_model_data(df, base_features, target)

            if data.empty:
                continue

            # Baseline на тех же датах
    ...
    for feature in determinants:
        for h in HORIZONS:
            for thr in TARGET_THRESHOLDS:
                ...
                # ВАЖНО:
                # common_features = SET0 + feature
                # Значит SET0 и SET0+feature сравниваются на одинаковых датах.
...
def make_feature_recommendations(deltas, periods):
    ...
    # Рекомендации делаем ТОЛЬКО по validation, чтобы не подгонять по test.
    selection = deltas[
    ...
    for feature, g in selection.groupby("feature"):
        ...
        # Временные интервалы по feature из periods
        ...
        if recommendation == "INCLUDE":
            ...
            explanation = (
                ...
                "Не удалять окончательно; проверить в группах/interaction."
            )
        elif recommendation == "WEAK_REVIEW_LOW_ROWS":
            explanation = (
                "Недостаточно наблюдений для уверенного вывода. "
                "Использовать осторожно или проверять на сокращённой выборке."
            )
        else:
            explanation = (
...
def plot_top_features(recommendations):
    ...
    plt.title("Top determinants by average validation delta ROC-AUC")
    ...
    plt.axvline(0, linewidth=1)
    plt.title("Top determinants by average validation delta balanced accuracy")
    ...
    plt.title("Feature recommendation counts")
...
def write_summary(base_results, deltas, recommendations, periods):
    ...
    with open(path, "w", encoding="utf-8") as f:
        f.write("SECTION 1.1 — BASELINE, SET0, SET0 + EACH DETERMINANT\n")
        f.write("=====================================================\n\n")
        ...
        f.write("2. SET0 = BTC own lags and rolling volatility.\n")
...
def write_excel_report(base_results, raw_results, periods, deltas, recommendations):
    ...
    with pd.ExcelWriter(xlsx_path, engine="openpyxl") as writer:
        ...
        if not raw_results.empty:
            # Полный raw может быть большим, но полезно сохранить
...
def main():
    ...
    print("Starting Section 1.1 pipeline...")
