"""
BTC Stage 3.0 — Final Walk-Forward Stability Check
==================================================
...
Запуск:
    python btc_3_0_walk_forward_final_check.py
"""
...
STAGE2_DIR = BASE_DIR / "btc_2_1_joint_model_outputs_fast_selection_final_heavy"
...
OUTPUT_DIR = BASE_DIR / "btc_3_0_walk_forward_final_check_outputs"
...
def get_models(y_train):
    ...
    return {k: v for k, v in models.items() if k in RUN_MODELS or k == "Baseline_MajorityClass"}
...
def make_plots(wf_summary, conf_summary, comparison):
    ...
    if wf_summary is not None and not wf_summary.empty:
        ...
        save_bar_top(
            ...
            "mean_roc_auc",
            "regime_model",
            "Stage 3.0 walk-forward: top regimes by mean ROC-AUC",
        ...
        save_bar_top(
            ...
            "regime_model",
            "Stage 3.0 walk-forward: top regimes by mean balanced accuracy",
        ...
        heat = (
            ...
            .max()
            .reset_index()
            .pivot(index="horizon", columns="target_threshold_pct", values="mean_roc_auc")
        )
        ...
        plt.title("Stage 3.0 WF heatmap: horizon x threshold")
        ...
        plt.title("Stage 3.0 stability flags")
    ...
    if conf_summary is not None and not conf_summary.empty:
        ...
        save_bar_top(
            ...
            "regime_model",
            "Stage 3.0 confidence: top regimes by signal accuracy",
        ...
        plt.title("Stage 3.0 confidence: mean signal accuracy vs coverage")
    ...
    if comparison is not None and not comparison.empty:
        ...
        if not comp.empty:
            ...
            plt.title("Stage 2 validation ROC-AUC vs Stage 3 walk-forward ROC-AUC")
...
def save_excel(
    ...
    xlsx_path = OUTPUT_DIR / "BTC_STAGE_3_0_WALK_FORWARD_FINAL_CHECK.xlsx"
    ...
    readme = pd.DataFrame({
        ...
        "text": [
            "Stage 3.0 — заключительная проверка устойчивости финальных режимов Stage 2.1.",
...
def make_recommended_final(wf_summary, conf_summary):
    ...
    # Колонки разные, concat сам поставит NaN где нет.
...
def write_summary_txt(wf_summary, conf_summary, recommended, warnings_list, plot_files, excel_path):
    path = OUTPUT_DIR / "BTC_STAGE_3_0_SUMMARY.txt"

    with open(path, "w", encoding="utf-8") as f:
        f.write("BTC STAGE 3.0 — WALK-FORWARD FINAL CHECK\n")
        f.write("=" * 80 + "\n\n")
...
def main():
    print("=" * 80)
    print("BTC Stage 3.0 — Walk-Forward Final Check")
    print("=" * 80)
    ...
    print("- BTC_STAGE_3_0_WALK_FORWARD_FINAL_CHECK.xlsx")
    print("- BTC_STAGE_3_0_SUMMARY.txt")
