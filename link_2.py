...
OUTPUT_DIR = BASE_DIR / "link_2_1_joint_model_outputs_fast_selection_final_heavy"
PLOTS_DIR = OUTPUT_DIR / "plots"

STAGE1_DIR = BASE_DIR / "link_1_1_baseline_feature_test_outputs"
...
GROUP_RU = {
    "SET0": "базовые признаки цены и волатильности LINK",
    "market": "рыночные признаки LINK",
    "behavior": "поведенческие признаки",
    "onchain": "ончейн-признаки LINK",
...
def load_ml_data():
    ...
    # LINK starts later; remove rows before the actual LINK sample start.
...
def build_candidate_pool_from_stage1():
    ...
    if not deltas_path.exists():
        raise FileNotFoundError(
            ...
            f"{deltas_path}. Сначала запусти этап 1.1 для LINK."
        )
    ...
    with pd.ExcelWriter(CANDIDATE_POOL_PATH, engine="openpyxl") as writer:
        readme = pd.DataFrame({
            ...
            "text": [
                "Автоматически собранный candidate pool для LINK Stage 2.1.",
                str(deltas_path),
...
def load_candidate_pool():
    ...
    # If candidate pool was created from an older ETH-style column name, map it to LINK column.
    cand["feature"] = cand["feature"].replace({
        "ETH_mcap_log_return": "link_mcap_log_return",
        "eth_mcap_log_return": "link_mcap_log_return",
        "LINK_mcap_log_return": "link_mcap_log_return",
    })
...
def run_stage2_pipeline(df, candidate_pool):
    ...
    for h in HORIZONS:
        for thr in TARGET_THRESHOLDS:
            ...
            print("\n" + "=" * 80)
            print(f"Running LINK stage 2.1 | horizon={h} | threshold={pct}%")
            print("=" * 80)
...
def write_excel_report(outputs):
    xlsx_path = OUTPUT_DIR / "2_1_LINK_joint_model_report_fast_selection_final_heavy.xlsx"
    ...
    with pd.ExcelWriter(xlsx_path, engine="openpyxl") as writer:
        readme = pd.DataFrame({
            ...
            "text": [
                "Это второй этап LINK: joint-модели по кандидатам из первого этапа.",
...
def write_text_summary(outputs):
    ...
    with open(path, "w", encoding="utf-8") as f:
        f.write("LINK — STAGE 2.1 JOINT MODEL PIPELINE\n")
        ...
        f.write("- 2_1_LINK_joint_model_report_fast_selection_final_heavy.xlsx\n")
...
def make_plots(outputs):
    ...
    if final_plan is not None and not final_plan.empty:
        ...
        plt.title("LINK stage 2.1: top regimes by validation score after combined stage")
    ...
    if confidence_selected is not None and not confidence_selected.empty:
        ...
        plt.title("LINK confidence regimes: validation accuracy vs coverage")
        plt.xlabel("Coverage")
...
def main():
    ...
    print("LINK Stage 2.1 joint pipeline")
    ...
    print("\nLINK feature groups found/missing:")
    ...
    print("\nMain files:")
    print("- 2_1_LINK_joint_model_report_fast_selection_final_heavy.xlsx")
