...
OUTPUT_DIR = Path(FILE_PATH).parent / "eth_1_1_baseline_feature_test_outputs"
...
def read_excel_with_fallback():
    """
    В твоей таблице первая строка может быть группировочной:
...
def create_targets(df):
    """
    target_return_h должен быть лог-доходностью ETH за h дней вперёд.
...
def run_individual_determinants(df):
    print("\n=== ETH 1.1B SET0 + each determinant ===")
...
def write_summary(base_results, deltas, recommendations, periods):
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        f.write("2. SET0 = ETH own lags and rolling volatility.\n")
